...
TypeStack.computeDimensions = function(
    ...
    if (
        !/\/topic\/image$/.test(propertyRecord.id) && 
        !/\/topic\/article$/.test(propertyRecord.id) && 
        propertyRecord.expectedType != null &&
        propertyRecord.expectedType != "/common/image" && 
        propertyRecord.expectedType != "/common/document" &&
...
TrailPoint.prototype.installUI = function() {try {
    ...
    div.innerHTML = 
        ...
        '<div class="facet-pane-status-section">' +
            '<img src="' + baseUrl + 'images/progress-running.gif" /> Suggesting useful filters...' +
        '</div>';
    ...
    div.innerHTML = 
        '<div class="pivot-pane-sign">' +
            '<img src="' + baseUrl + 'images/parallax-sign-white.png" />' +
        '</div>' +
        ...
        '<div class="pivot-pane-status-section status">' +
            '<img src="' + baseUrl + 'images/progress-running.gif" /> Finding connections...' +
        '</div>' +
...
function TopicPageRendering(itemID, outerDiv, onDone, focusHandler, pivotHandler) {
    ...
    var self = this;
    var url = "http://hotshot.jdouglas.user.dev." + window.ParallaxConfig.appBaseUrl + "json?id=" + encodeURIComponent(itemID);
        ...
        if ("id" in valueEntry) {
            ...
            if ("/common/topic/image" in valueEntry) {
                ...
                if (valueEntry["/common/topic/image"] != null) {
                    ...
                    img.src = baseUrl + "images/blank-16x16.png";
...
function renderItem(itemID, div, onDone, onFocus) {
    ...
    var url = "http://hotshot.dfhuynh.user.dev." + window.ParallaxConfig.appBaseUrl + "json?id=" + encodeURIComponent(itemID);
    ...
    var dom = {
        ...
        appendMenuItem: function(label, icon, onClick) {
            ...
            divItem.appendChild(SimileAjax.Graphics.createTranslucentImage(
                icon != null ? icon : (baseUrl + "images/blank-16x16.png")));
...
SingleTopicView.prototype._constructUI = function() {
    ...
    this._div.innerHTML = '<div class="status"><img src="' + baseUrl + 'images/progress-running.gif" /> Retrieving information on ' + itemID + '...</div>';
...
function timelineViewFillEventSource(eventSource, points) {
    ...
    var addEvent = function(point) {
        ...
        var evt = new Timeline.DefaultEventSource.Event( {
                      id: point.itemID,
                   start: point.start,
                     end: "end" in point ? point.end : null,
             latestStart: null,
             earliestEnd: null,
                 instant: !("end" in point),
                    text: point.itemName,
             description: "",
                   image: null,
                    link: null,
                   ...
                   color: color,
               textColor: color,
               hoverText: point.itemID,
               classname: null,
               tapeImage: null,
              tapeRepeat: null,
                 caption: null,
                 eventID: point.itemID,
                trackNum: null
        });
...
ViewUtil.renderViewSummary = function(collection, div) {
    ...
    div.innerHTML = '<div class="status"><img src="' + baseUrl + 'images/progress-running.gif" /> Retrieving information...</div>';
